...
# In the first case, the difference between the max
# and min elements of the window is two (3 - 1 == 2). 
# In the second case, that difference is three (5 - 2 == 3). 
# We want to write a function that will 
# return the larger of these two differences.
...
# max elements of each window (each method is an O(n) operation). 
# If we use the min AND max methods built into Ruby, this costs us 
# 2 * window_size iterations for each window (overall time complexity: O(n^2)).

# What's more, in the naive solution, we consider
# each window as a slice of the input array. 
# On the first iteration, we slice the array from index 0 to index w. 
# On the second iteration, we slice from 1 to w + 1, and so forth. 
# However, slicing an array is rather costly (again, O(n)). 
# Remember, a new array is created when slicing an existing array.

# What if it were possible to calculate min and 
# max instantaneously (in constant time) per window? 
# This would allow us to find the max windowed range in O(n) time. 
# We can achieve this by writing a custom data structure
# dedicated to solving this specific problem.

# Optimized Solution:
# We will be creating a sequence of data structures that will culminate 
# in a MinMaxStackQueue, our custom data structure that 
# will keep track  of the min and max in constant time.
...
# Every time we move the window, we could enqueue 
# the next element and dequeue the last element. 
# This would allow us to avoid using Array#slice, 
# so that we can traverse the array in constant time.

# A queue is a simple abstract linear data structure where elements
# are stored in order and can be added or removed one at a time.
# A queue follows first in, first out (FIFO). Unlike Ruby's Array 
# data structure, most Queue implementations do not expose methods
# to slice or sort the data, or to find a specific element. 
...
# We will also write a peek method, which returns the
...
# data store, as long as you don't expose it publicly (i.e., don't 
# define an attr_reader for it). We'll do this in today's exercises.
...
# which means we cannot make use of Array#slice, and
# each window must calculate the min and max instantly.
# Every time we move the window, we enqueue the
# next element and dequeue the last element. 
# This solves the problem with slice. 
# However, removing items from MyQueue takes O(n) time. 
# As the first element of the array is shifted off, the 
# remaining elements will be reassigned in new position in memory.
# Also, it still leaves us with the problem of expensive min and max operations.
# To resolve this, we'll have to make clever 
# use of another data structure, the stack.

# Stacks are another simple linear data structure.
# Elements are also stored in order and can be added or removed one at a time.
# A stack is first in, last out (FILO).
# Similar to queues, stack implementations do not expose methods to slice
# or sort the data, or to find a specific element.
...
# Done properly, we will still have a queue but
# with the advantages of dequeuing in O(1) time.

# Before you start to code this, sit down and talk to your partner about
# how you might implement this. You should not modify your MyStack class,
# but use the interface it provides to implement a queue.

# When you're ready, implement this StackQueue class 
# with size, empty?, enqueue, and dequeue methods.

# Hint: You will want to use more than one instance of MyStack.

# Hint 2: What if you always pushed onto one stack, and always 
# popped from the other? How will these two stacks interact?
...
# However, this requires us to iterate over every item in 
# the @store array, which gives us a time complexity of O(n).
# This isn't good enough for us; we want to be 
# able to return the max in constant time (O(1)).
# If we can't iterate over @store, how else could
# we modify the stack to get this functionality?
...
# In other words, we can be storing more information
...
# but use our new data structure instead of array slices.
# As before, return the current_max_range at the end of the method.
...
puts windowed_max_range([1, 0, 2, 5, 4, 8], 3) == 5 # 0, 2, 5 # true
puts windowed_max_range([1, 0, 2, 5, 4, 8], 4) == 6 # 2, 5, 4, 8 # true
puts Benchmark.measure { windowed_max_range([1, 3, 2, 5, 4, 8], 5) } # == 6 # 3, 2, 5, 4, 8 # true
